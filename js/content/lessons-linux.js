...
(function (root) {
...

  {
    ...
    goal: 'Понять план четырёх дней и правило «руками, без копирования»',
    mode: 'shell',
    theory: `
<p>Курс собран под одну конкретную цель: <b>пройти техническое собеседование на QA-инженера тестирования ядра мобильной сети</b>. Всё лишнее выброшено.</p>
<table class="tbl">
...
<p class="note">В квестах стенд специально запускается со сломанным состоянием — ваша задача найти причину и починить, как на работе. В конце курса есть кнопка <b>🎯</b> — режим собеседования: 10 вопросов без подсказок, с таймером.</p>`,
...

  {
    ...
    goal: 'Понять три команды, с которых начинается любая работа на чужом сервере',
    mode: 'shell',
    theory: `
...
<p class="note">На стенде уже развёрнут учебный узел ядра: сервис <code>core-registrar</code>, конфиг в <code>/etc/core/</code>, логи в <code>/var/log/core/</code>. Дальше весь курс идёт вокруг него.</p>`,
    task: 'Перейдите в каталог <code>/etc/core</code> и посмотрите его содержимое подробно (с правами и размерами).',
    checks: [
    ...
    solution: 'cd /etc/core\nls -la',
    praise: 'Так начинается любая смена: где я, что тут лежит.',
    quiz: {
